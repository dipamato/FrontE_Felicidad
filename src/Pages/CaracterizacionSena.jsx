    ...
    if (!nombreOtraInstitucion) {
      async function cargarOpcionesLista2() {
        const response = await axios.get(
          `${apiUrl}/institucion/${"SENA - REGIONAL CALDAS"}`
        );
        setOpcionesLista2([response.data]);
      }
      if ("SENA - REGIONAL CALDAS") {
  ...
  function handleClick() {
    if (validateForm()) {
      ...
      if (nombreOtraInstitucion) {
        ...
        data = { Institucion: "SENA - REGIONAL CALDAS", Sede: seleccionLista2 };
      }
...
export default CaracterizacionSena;
